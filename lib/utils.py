...
# Datos observados hasta el último ECG normal antes de uno con fibrilación
# Datos a predecir, los que van a continuación    
	n_pacientes = data_dict['data'].size(0)
	n_time_steps = data_dict['data'].size(1)
	n_observed_tp = []
	n_tp_to_predict = []
	for i in range(n_pacientes):
		pac_fib = 0
		for j in range(n_time_steps):
			if data_dict['labels'][i][j] == 1:
				pac_fib = 1
		if pac_fib == 0:
			for j in range(n_time_steps):
				if torch.isnan(data_dict['labels'][i][j]) == False:
					n_observed_tp.append(j)
			n_tp_to_predict.append(n_observed_tp[-1])
			n_observed_tp = n_observed_tp[:-1]
			n_tp_to_predict.append(n_observed_tp[-1])
			continue
		for j in range(n_time_steps):
			if data_dict['labels'][i][j] == 1:
			...
			if torch.isnan(data_dict['labels'][i][j]) == False :
				n_observed_tp.append(j)
		for j in range(j, n_time_steps):
			if torch.isnan(data_dict['labels'][i][j]) == False :
				n_tp_to_predict.append(j)
                
		n_tp_to_predict.append(n_observed_tp[-1])
	n_observed_tp.sort()
	n_tp_to_predict.sort()
...
def compute_loss_all_batches(model,
	...
	run_backwards = True # Valor que debería figurar por defecto
	if args.extrap:
		run_backwards = False
	...
	for i in range(n_batches):
		...
		results  = model.compute_all_losses(batch_dict,
			n_traj_samples = n_traj_samples, kl_coef = kl_coef, run_backwards = run_backwards)
